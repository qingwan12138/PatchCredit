...
# 17. 预实验本地模型冻结：Qwen2.5-Coder-3B-Instruct
...
Deployment: local
GPU: RTX 4090 24GB
Role: LLM Compiler Agent / Plan Generator
```

该模型在 PatchCredit 中**只负责生成初始编译优化计划**，不负责最终性能判断。

固定职责：

```text
Benchmark / IR Summary
+
Allowed LLVM Pass Catalog
+
Optimization Objective
↓
Qwen2.5-Coder-3B-Instruct
↓
4–8 action compiler optimization plan
```
...
> 模型“认为某个 Plan 更快”不构成性能证据，真正性能只由真实编译和 Runtime 测量决定。

## 为什么预实验固定 3B

预实验目标是验证 PatchCredit 是否存在真实研究现象，而不是追求最强 LLM 本身。
...
1. 本地部署简单
2. RTX 4090 运行负担低
3. 可以低成本生成几十到上百个 Plan
4. 模型固定后，实验变量更少
5. 即使 3B 生成的 Plan 不完美，也正适合验证 PatchCredit 是否能诊断和修复
```
...
只有在 P3/P4 出现以下情况之一时，才允许升级：

```text
A. JSON / Plan 解析成功率长期过低
B. 绝大多数 Plan 无法编译
C. 绝大多数 Plan 无正确性
D. 几乎无法得到任何 Runtime 有效提升 Plan
```
...
```text
Qwen2.5-Coder-3B-Instruct = 固定主模型
```
...
但 PatchCredit 的论文贡献不能建立在“大模型越强”上，而应建立在：

```text
对任意 LLM-generated compiler plan 的性能归因与二次优化
```
